...
function createGoogleMapsAPIInitializer() {
     ...
     */
    const googleMapsAPIInitializer = (options, loadCn = false) => {
        /**
        ...
        if (!isApiSetUp) {
            isApiSetUp = true;
            const baseUrl = typeof loadCn === 'boolean' && loadCn
            ...
            const query = Object.keys(finalOptions)
                .map((key) => {
                if (finalOptions[key]) {
                    return `${encodeURIComponent(key)}=${encodeURIComponent(finalOptions[key])}`;
                }
                return '';
            })
                .join('&');
