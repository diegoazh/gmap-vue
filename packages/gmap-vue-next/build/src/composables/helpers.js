...
export function getPropsValues(vueInst, props) {
    return Object.keys(props).reduce((acc, prop) => {
        if (vueInst.$props[prop] != null) {
            acc[prop] = vueInst.$props[prop];
        }
        return acc;
    }, {});
}
...
export function bindProps(vueInst, googleMapsInst, props) {
    Object.keys(props).forEach((attribute) => {
        ...
        if (!noBind) {
            ...
            const initialValue = vueInst.$props[attribute];
            ...
            if (type !== Object || !trackProperties) {
                ...
                vueInst.$watch(attribute, () => {
                    const attributeValue = vueInst.$props[attribute];
                ...
                watchPrimitiveProperties(vueInst, trackProperties.map((prop) => `${attribute}.${prop}`), () => {
                    googleMapsInst[setMethodName](vueInst.$props[attribute]);
                }, vueInst.$props[attribute] !== undefined);
            }
            if (twoWay &&
